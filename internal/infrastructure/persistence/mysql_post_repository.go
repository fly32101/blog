...
func (r *MySQLPostRepository) Create(post *entity.Post) error {
	query := `INSERT INTO posts (title, content, author, view_count, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)`
	result, err := r.db.Exec(query, post.Title, post.Content, post.Author, post.ViewCount, post.CreatedAt, post.UpdatedAt)
	if err != nil {
...
func (r *MySQLPostRepository) GetByID(id uint) (*entity.Post, error) {
	query := `SELECT id, title, content, author, view_count, created_at, updated_at FROM posts WHERE id = ?`
	row := r.db.QueryRow(query, id)

	post := &entity.Post{}
	err := row.Scan(&post.ID, &post.Title, &post.Content, &post.Author, &post.ViewCount, &post.CreatedAt, &post.UpdatedAt)
	if err != nil {
...
func (r *MySQLPostRepository) GetAll() ([]*entity.Post, error) {
	query := `SELECT id, title, content, author, view_count, created_at, updated_at FROM posts ORDER BY created_at DESC`
	...
	for rows.Next() {
		post := &entity.Post{}
		err := rows.Scan(&post.ID, &post.Title, &post.Content, &post.Author, &post.ViewCount, &post.CreatedAt, &post.UpdatedAt)
		if err != nil {
...
func (r *MySQLPostRepository) Update(post *entity.Post) error {
	query := `UPDATE posts SET title = ?, content = ?, view_count = ?, updated_at = ? WHERE id = ?`
	_, err := r.db.Exec(query, post.Title, post.Content, post.ViewCount, time.Now(), post.ID)
	if err != nil {
...
func (r *MySQLPostRepository) GetByCategory(categoryID uint) ([]*entity.Post, error) {
	query := `
		SELECT p.id, p.title, p.content, p.author, p.view_count, p.created_at, p.updated_at 
		FROM posts p
