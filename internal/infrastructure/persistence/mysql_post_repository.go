...
func (r *MySQLPostRepository) Create(post *entity.Post) error {
	...
	result, err := r.db.Exec(query, post.Title, post.Content, post.Author, post.CreatedAt, post.UpdatedAt)
	if err != nil {
...
func (r *MySQLPostRepository) GetByID(id uint) (*entity.Post, error) {
	query := `SELECT id, title, content, author, created_at, updated_at FROM posts WHERE id = ?`
...
func (r *MySQLPostRepository) Update(post *entity.Post) error {
	...
	_, err := r.db.Exec(query, post.Title, post.Content, time.Now(), post.ID)
	if err != nil {
...
func (r *MySQLPostRepository) GetByCategory(categoryID uint) ([]*entity.Post, error) {
	query := `
		SELECT p.id, p.title, p.content, p.author, p.created_at, p.updated_at 
		FROM posts p
